...
function finalize() {
  ...
  var leftAvg = results.slice(0, results.length / 2).reduce(function (sum, n) { return sum + n }, 0) / (results.length / 2);
  var rightAvg = results.slice(results.length / 2).reduce(function (sum, n) { return sum + n }, 0) / (results.length / 2);
  var dataToSave = {
    date: Date.now(),
